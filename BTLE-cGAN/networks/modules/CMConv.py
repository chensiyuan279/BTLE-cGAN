...
class CMConv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1, dilation=3, groups=1, dilation_set=4,
        ...
        # Zero out weights with mask
        self.conv.weight.data[self.mask] = 0

        # Register backward hook
        self.conv.weight.register_hook(self.backward_hook)

        self.groups = groups

    def backward_hook(self, grad):
        if not isinstance(self.mask, torch.BoolTensor):
            self.mask = self.mask.to(torch.bool)  # Ensure mask is boolean
        out = grad.clone()
        out[self.mask] = 0
        return out
