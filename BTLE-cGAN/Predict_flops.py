...
def compute_flops(model, img1, img2):
    """
    计算模型的浮点操作数（FLOPS）。
    参数:
    - model: PyTorch 模型
    ...
    def count_flops(module, input, output):
        ...
        if isinstance(module, nn.Conv2d):
            ...
            out_channels, _, kernel_height, kernel_width = module.weight.shape
            flops += 2 * batch_size * out_channels * in_channels * kernel_height * kernel_width * height * width
    ...
    with torch.no_grad():
        model(img1, img2)  # Pass both img1 and img2
...
def predict_and_save(model, dataloader, save_dir):
    ...
    model.eval()
    total_flops = 0  # 记录总的 FLOPS
    with torch.no_grad():
        for i, (x, y, img1, img2) in enumerate(dataloader):
            img1 = img1.to(device)
            img2 = img2.to(device)

            # 计算每个批次的 FLOPS，传递 img1 和 img2
            flops = compute_flops(model, img1, img2)
            total_flops += flops

            file_name = os.path.basename(x[0])  # 获取文件名
            # 生成预测
            ...
            # 打印每批次的 FLOPS
            print(f"Batch {i + 1}: FLOPS = {flops:.2f}")

    print(f"Total FLOPS for all batches: {total_flops:.2f}")


# 运行预测并保存图像
save_dir = r"/root/autodl-tmp/data/test2/23_24testflops" # 预测图像的保存路径
